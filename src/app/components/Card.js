...
function Cards({key, data, stringify, view}) {
  ...
  useEffect(() => {
    ...
    if(view==='list'){
      card.current.classList.add(['flex'], ['border'], ['rounded-2xl'], ['items-center'], ['justify-start'], ['gap-2'], ['py-3'], ['px-2']);
      nameType.current.classList.add(['flex'], ['flex-col']);
      image.current.classList.add(['w-1/3']);
    }
  }, [view])
  ...
  return (
    <a className='py-2 hover:-translate-y-2 duration-100' key={key} href="#" ref={card}>
